...
final class ControllerExtensionPaymentPaymentGatewayCloudCreditCard extends Controller
{
    ...
    public function confirm()
    {
        ...
        try {
            ...
            $debit->setSuccessUrl(str_replace('&amp;', '&', $this->url->link('extension/payment/payment_gateway_cloud_' . $this->type . '/response', ['orderId' => $orderId, 'success' => 1])));
            $debit->setCancelUrl(str_replace('&amp;', '&', $this->url->link('extension/payment/payment_gateway_cloud_' . $this->type . '/response', ['orderId' => $orderId, 'cancelled' => 1])));
            $debit->setErrorUrl(str_replace('&amp;', '&', $this->url->link('extension/payment/payment_gateway_cloud_' . $this->type . '/response', ['orderId' => $orderId, 'failed' => 1])));
            $debit->setCallbackUrl(str_replace('&amp;', '&', $this->url->link('extension/payment/payment_gateway_cloud_' . $this->type . '/callback', 'orderId=' . $orderId, '&cardType=' . $cardType)));
